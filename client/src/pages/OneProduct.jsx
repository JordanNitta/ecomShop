        ...
        <div className=' bg-white pt-[40px]'>
            ...
            <div className='flex-col '>
                <div className='flex justify-center'>
                    <img src={product.image} alt={product.name} width={450} className='md:w-['/>
                </div>
                <div className='flex justify-center mt-[30px]'>
                    <h1 className='hidden lg:block lg:text-[16px] lg:font-500 lg:text-gray-500'>{product.brand}</h1>
                    <h1 className="hidden lg:block lg:mb-2 lg:text-[30px] lg:font-semibold lg:text-black">{product.name}</h1>
                    <div className='flex flex-col w-[80%] md:justify-center md:items-center md:w-[50%] gap-5'>
                        <h1 className='text-[16px] upper font-semibold text-left'>Select a Size</h1>
                        {product && product.size ? (
                            <div className='hidden lg:grid lg:grid-cols-5 lg:gap-[5px] lg:mt-3 '>
                                {product.size.map((size, index) => (
                                    <button
                                        className={selectedSize === size ? 'font-style text-[14px] font-semibold border-[1px] border-black ' : 'font-style text-[14px] font-semibold border-[1px] border-gray-200 p-3'}
                                        onClick={() => handleBorder(size)}
                                        key={index}
                                    >
                                        {size}
